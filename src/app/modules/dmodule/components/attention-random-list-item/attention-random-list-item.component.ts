...
@Component({
  selector: 'app-attention-random-list-item',
  templateUrl: './attention-random-list-item.component.html',
  styleUrls: ['./attention-random-list-item.component.scss'],
})
export class AttentionRandomListItemComponent implements OnInit {
  xmlTag = 'attentionRandomListItemTemp';
  text = '';
  @Input('dataSource') dataSource;
  @ViewChild('attentionRandomListItemTemp', {static: true, read: ViewContainerRef}) attentionRandomListItemTemp: ViewContainerRef;
  constructor(
  ...
  createComponent(node, nodeComponent) {
    ...
    const nodeCp = this.attentionRandomListItemTemp.createComponent(nodeComp);
