...
@Component({
  selector: 'app-dm-ref',
  templateUrl: './dm-ref.component.html',
  styleUrls: ['./dm-ref.component.scss'],
  providers: [DynamicDmComponentService]
})
export class DmRefComponent implements OnInit {
  xmlTag = 'action';
  text = '';
  @Input('dataSource') dataSource;
  @ViewChild('dmRefTemp', {static: true, read: ViewContainerRef}) dmRefTemp: ViewContainerRef;
  constructor(
  ...
  createComponent(node, nodeComponent) {
    ...
    const nodeCp = this.dmRefTemp.createComponent(nodeComp);
