...
@Component({
  selector: 'app-referenced-applic-group',
  templateUrl: './referenced-applic-group.component.html',
  styleUrls: ['./referenced-applic-group.component.scss'],
  providers: [DynamicDmComponentService]
})
export class ReferencedApplicGroupComponent implements OnInit {
  xmlTag = 'action';
  text = '';
  @Input('dataSource') dataSource;
  @ViewChild('referencedApplicGroupTemp', {static: true, read: ViewContainerRef}) referencedApplicGroupTemp: ViewContainerRef;
  constructor(
  ...
  createComponent(node, nodeComponent) {
    ...
    const nodeCp = this.referencedApplicGroupTemp.createComponent(nodeComp);
