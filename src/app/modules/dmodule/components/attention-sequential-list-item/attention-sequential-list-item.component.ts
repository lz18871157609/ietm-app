...
@Component({
  selector: 'app-attention-sequential-list-item',
  templateUrl: './attention-sequential-list-item.component.html',
  styleUrls: ['./attention-sequential-list-item.component.scss'],
})
export class AttentionSequentialListItemComponent implements OnInit {
  xmlTag = 'attentionSequentialListItemTemp';
  text = '';
  @Input('dataSource') dataSource;
  @ViewChild('attentionSequentialListItemTemp', {static: true, read: ViewContainerRef}) attentionSequentialListItemTemp: ViewContainerRef;
  constructor(
  ...
  createComponent(node, nodeComponent) {
    ...
    const nodeCp = this.attentionSequentialListItemTemp.createComponent(nodeComp);
